...
| READY_TO_DELETE | **手動操作** -- 管理員確認預覽後手動設定。下次執行時依序清除專案內所有資源（VM、磁碟區、網路、路由器、浮動 IP、安全群組、快照、負載平衡器、映像、Ceph RadosGW 物件儲存 Bucket 與其物件），再刪除 OpenStack Group（移除所有成員）與專案。使用者僅在無其他專案角色時才會刪除；若該使用者在其他專案仍有角色指派，則僅移除目標專案上的角色，保留使用者帳號 |
...
│   └── csv_repository.py
├── services/
│   ├── openstack_service.py  # 使用者/專案/Group/配額管理、資源清單、刪除預覽（含 RGW Bucket）、跨專案安全刪除
│   ├── rgw_admin.py          # Ceph RadosGW Admin REST API 客戶端（AWS Sig V4 認證）；可跨任意 implicit-tenant 專案列舉與刪除 Bucket
