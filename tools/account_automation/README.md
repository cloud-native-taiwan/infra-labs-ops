...
| PENDING_DELETE | 預覽即將刪除的資源（使用者、專案、VM、Volume），寄送預覽信給管理員 |
| READY_TO_DELETE | **手動操作** -- 管理員確認預覽後手動設定。下次執行時刪除 OpenStack 使用者與專案 |
...
├── models.py            # 凍結資料類別、Status 列舉、DeletePreview
...
│   └── csv_repository.py
├── services/
│   ├── openstack_service.py  # 使用者/專案/配額管理、刪除預覽
